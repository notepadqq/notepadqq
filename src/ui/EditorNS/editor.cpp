...
void Editor::fullConstructor(const Theme& theme)
{
    ...
    // QString content = QString("<html><body onload='setTimeout(function() { window.location=\"%1\"; },
    // 1);'>Loading...</body></html>").arg("file://" + Notepadqq::editorPath());
...
{
    return asyncSendMessageWithResultP("C_FUN_IS_CLEAN", QVariant(0)).then([](QVariant v) { return v.toBool(); });
}
...
{
    return asyncSendMessageWithResultP("C_FUN_GET_HISTORY_GENERATION").then([](QVariant v) { return v.toInt(); });
}
...
{
    return asyncSendMessageWithResultP("C_CMD_SET_INDENTATION_MODE", QVariantMap{{"useTabs", useTabs}, {"size", size}})
        .then([]() {});
}
...
std::shared_future<QVariant> Editor::asyncSendMessageWithResult(
    const QString msg, std::function<void(QVariant)> callback)
...
{
    return asyncSendMessageWithResultP("C_FUN_GET_CURSOR").then([](QVariant v) {
...
QList<Selection> Editor::selections()
{
    QList<Selection> out;
...
{
    return asyncSendMessageWithResultP("C_FUN_GET_SELECTIONS_TEXT").then([](QVariant text) {
...
{
    return asyncSendMessageWithResultP("C_FUN_GET_CURRENT_WORD").then([](QVariant v) { return v.toString(); });
}

QtPromise::QPromise<int> Editor::lineCount()
{
    return asyncSendMessageWithResultP("C_FUN_GET_LINE_COUNT").then([](QVariant v) { return v.toInt(); });
}
} // namespace EditorNS
