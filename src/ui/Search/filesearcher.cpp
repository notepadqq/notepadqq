...
QRegularExpression FileSearcher::createRegexFromConfig(const SearchConfig& config)
{
    ...
    const QFlags<QRegularExpression::PatternOption> options =
        config.matchCase ? QRegularExpression::MultilineOption
...
DocResult FileSearcher::searchPlainText(const SearchConfig& config, const QString& content)
{
    ...
    while ((offset = content.indexOf(searchString, offset, caseSense)) != -1) {
        ...
        // std::upper_bound returns an iterator to the first item greater than 'offset'. This is the line after the
        // match. Subtract 1 to get the line the match is on.
...
void FileSearcher::run()
{
    ...
    const QFlags<QDirIterator::IteratorFlag> dirIteratorOptions =
        m_searchConfig.includeSubdirs ? (QDirIterator::Subdirectories | QDirIterator::FollowSymlinks)
    ...
    for (const auto& fileName : fileList) {
        ...
        if (decodedText.error) {
            // File could not be read. We'll ignore this error since it should never happen. QDirIterator only iterates
            // over readable files and DocEngine only reads the file. But if it happens we can skip the rest, just in
            // case.
            continue;
        }
