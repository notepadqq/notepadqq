...
QString getFormattedLocationText(const DocResult& docResult, const QString& searchLocation)
{
    ...
    const QString relativePath =
        docResult.fileName.startsWith(searchLocation) ? docResult.fileName.mid(commonPathLength) : docResult.fileName;
...
{
    m_treeWidget->topLevelItem(0)->setText(
        0, QString(tr("Search in progress [%1/%2 finished]")).arg(processed).arg(total));
}
