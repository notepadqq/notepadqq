...
{
    connect(object, &QObject::destroyed, this, [&] { m_unmanagedPointer = nullptr; });
}
...
{
    if (m_pointerType == PointerType::DETACHED) {
        ret = Stub::StubReturnValue(
            ErrorCode::METHOD_NOT_FOUND, QString("Method not found for %1: %2").arg(stubName_(), method));
        return false;
    } else {
        ...
        for (int i = 0; i < methodCount; i++) {
            ...
            QVariant retval = genericCall(obj, metaMethod, args.toVariantList(), err);
        ...
        ret = Stub::StubReturnValue(ErrorCode::METHOD_NOT_FOUND,
            QString("Method not found for %1: %2 with %3 args").arg(stubName_(), method).arg(args.count()));
        return false;
    }
}
...
QVariant Stub::genericCall(QObject* object, QMetaMethod metaMethod, QVariantList args, ErrorCode& error)
{
    ...
    if (QString(metaMethod.typeName()) != "void") {
        returnValue = QVariant(QMetaType::fromName(metaMethod.typeName()), static_cast<void*>(NULL));
    }
...
{
    return m_pointerType == other.m_pointerType && m_sharedPointer == other.m_sharedPointer &&
           m_weakPointer == other.m_weakPointer && m_unmanagedPointer == other.m_unmanagedPointer &&
           m_rts == other.m_rts && stubName_() == other.stubName_();
}
