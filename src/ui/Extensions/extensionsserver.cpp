...
void ExtensionsServer::on_clientMessage(QLocalSocket* socket)
{
    ...
    while (1) {
        ...
        QString jsonResponse = QString(QJsonDocument(response).toJson(QJsonDocument::Compact)).trimmed();
...
{
    QString jsonMessage = QString(QJsonDocument(message).toJson(QJsonDocument::Compact)).trimmed();
