...
void MainWindow::openNewFile(QStringList fileNames)
{
    if(!fileNames.isEmpty())
    {
        ...
        for (int i = 0; i < fileNames.count(); i++) {
            ...
            // If there is only a new empty tab opened, remove it
            if(tabWidget1->count() == 1 && isNewEmptyTab(0)) {
...
{
    // Ask for file names...
    QStringList fileNames = QFileDialog::getOpenFileNames(0, tr("Open"), settings->value("lastSelectedDir", ".").toString(), tr("All files (*)"), 0, 0);
...
int MainWindow::addEditorTab(bool setFocus, QString title, QTabWidgetqq * _tabWidget)
{
    ...
    connect(sci, SIGNAL(fileChanged(QString)), SLOT(fileChanged(QString)));
...
void MainWindow::autoLexer(QString filename, QsciScintillaqq* parent)
{
...
{
    QString filename = this->getCurrentTextBox(tabWidget1)->fileName();
    if(filename != "")
    {
        int ret = QMessageBox::Yes;

        if(getCurrentTextBox(tabWidget1)->isModified())
        {
        ...
        if(ret == QMessageBox::Yes) {
            ...
            QsciScintillaqq* sci = this->getCurrentTextBox(tabWidget1);
...
int MainWindow::askIfWantToSave(int index)
{
    QMessageBox msgBox;
    QString file;
    QsciScintillaqq* sci = getTextBoxFromIndex(index, tabWidget1);
    ...
    if (sci->fileName() == "")
    {
        file = tabWidget1->tabText(index);
    } else {
        file = QFileInfo(sci->fileName()).fileName();
    }
    msgBox.setWindowTitle(QCoreApplication::applicationName());
    msgBox.setText(tr("Save file \"%1\"?").arg(file));
...
{
    //sender()->parent()
    //QMessageBox::information(this,"File changed :: debug",path,QMessageBox::Ok);
}
...
void MainWindow::on_scintillaSelectionChanged()
{
    ...
    QString selExtended = sci->text().mid(sci->positionFromLineIndex(lineFrom, indexFrom) - 1, sci->selectedText().length() + 2);
    bool isWord = true;
//    if(sci->isWordCharacter(selExtended.at(0).unicode()) || sci->isWordCharacter(selExtended.at(selExtended.length()-1).toAscii()))
//    {
//        isWord = false;
//    }

    if(isWord  &&  lineFrom == lineTo  &&  sci->hasSelectedText()  &&  sci->selectedText() != "")
    {
